...
// ── Pakistan Time Helpers (BULLETPROOF UTC MATH) ──
...
const markAttendance = async (req, res) => {
    ...
    try {
        ...
        /*
        if (currentTimeStr < "09:00" || currentTimeStr > "17:00") {
            return res.status(400).json({ 
                message: "Attendance can only be marked during office hours (09:00 AM to 05:00 PM)." 
            });
        }
        */
    ...
    try {
        ...
        if (employeeID) {
            ...
            if (employeeObj) {
                ...
                return res.status(200).json({ attendance: [], employeeCreatedAt: null, defaultAbsentDeduction: 0 });
            }
        }
        ...
        if (employeeID && employeeObj) {
            ...
            for (let i = 1; i <= daysInMonth; i++) {
                ...
                const exists = results.find(a => {
                    ...
                    return aPkt.getUTCDate() === i && (aPkt.getUTCMonth() + 1) === pm && aPkt.getUTCFullYear() === py;
                });
                ...
                if (!exists) {
                    ...
                    results.push({
                        date: virtualDate,
                        checkInTime: null, // ✅ FIX: explicit null taake frontend crash na ho
                        status: 'absent',
                        deduction: defaultAbsentDeduction,
                        employeeId: { EmployeeName: employeeObj.EmployeeName, employeeID: employeeID }
                    });
                }
            }
        }
